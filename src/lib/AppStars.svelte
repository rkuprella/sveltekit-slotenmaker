...
<div class="flex {small ? '' : 'space-x-2'}">
  {#each Array(parseInt(totalStars, 10)) as _, i}
    <svg
      class="{small ? 'w-4 h-4' : 'w-8 h-8'} text-yellow-400"
      fill="currentColor"
      viewBox="0 0 20 20"
  ...
  {/each}
  {#each Array(5 - parseInt(totalStars, 10)) as _, i}
    <div class="relative">
      {#if 5 - parseInt(totalStars, 10) + i + 1 < totalStars && totalStars % 1 != 0}
      ...
      {/if}
      <svg
        class="{small ? 'w-4 h-4' : 'w-8 h-8'} text-gray-300"
        fill="currentColor"
        viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg">
        <path
